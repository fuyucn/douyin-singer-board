...
- `main` is the canonical source for all common code and features. `feat/susu` tracks `main` and only adds SUSU-only customizations: `kugou_enabled` is locked ON (the About toggle is disabled), the default live room id `767116735823`, and SUSU-only assets; on `main`, the same code ships with `kugou_enabled` defaulting to OFF and an empty live room id.
...
- `feat/susu` must track `main`'s code and features; only the `-susu.<x>` version suffix, the locked-on `kugou_enabled` default, the default live room id `767116735823`, and SUSU-only customizations may differ.
